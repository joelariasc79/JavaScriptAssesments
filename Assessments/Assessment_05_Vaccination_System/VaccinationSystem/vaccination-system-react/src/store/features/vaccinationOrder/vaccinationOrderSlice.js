...
export const markOrderAsVaccinated = createAsyncThunk(
    ...
    // Ensure vaccinationDate is passed directly as the argument
    async ({ orderId, vaccinationDate }, { rejectWithValue }) => {
        try {
            // FIX: Pass vaccinationDate directly as the second argument to apiService.markOrderAsVaccinated
...
export const cancelOrderByPatient = createAsyncThunk(
    ...
    async (orderId, { rejectWithValue }) => {
        try {
            ...
            const message = error.response?.data?.message || error.message;
            return rejectWithValue(message); // [cite: 137]
        }
    }
);
...
const vaccinationOrderSlice = createSlice({
    ...
    orders: [],
        ...
        markVaccinatedEmailStatus: undefined, // New: State to hold email sending status for mark vaccinated action
        markVaccinatedEmailMessage: null,     // New: State to hold email sending message for mark vaccinated action
        ...
        vaccinatedPersons: [],
        fetchVaccinatedPersonsStatus: 'idle',
        fetchVaccinatedPersonsError: null,
},
reducers: {
    ...
    clearMarkVaccinatedStatus(state) {
        ...
        state.markVaccinatedEmailStatus = undefined; // Reset email status
        state.markVaccinatedEmailMessage = null;     // Reset email message
    },
    ...
    clearFetchVaccinatedPersonsStatus(state) {  
...
extraReducers: (builder) => {
    builder
        ...
        .addCase(markOrderAsVaccinated.pending, (state) => { // [cite: 154]
        state.markVaccinatedStatus = 'loading';
        state.markVaccinatedError = null;
        state.markVaccinatedEmailStatus = undefined; // Reset email status on pending
        state.markVaccinatedEmailMessage = null;     // Reset email message on pending
    })
        ...
        // Set email status and message from backend response
        state.markVaccinatedEmailStatus = action.payload.emailStatus;
        state.markVaccinatedEmailMessage = action.payload.message;
    })
        ...
        // Explicitly set email status to 'failed' if the API call itself failed (e.g., 500 error)
        // This ensures the UI always gets a 'failed' status for email in case of thunk rejection.
        state.markVaccinatedEmailStatus = 'failed';
        state.markVaccinatedEmailMessage = action.payload.message || 'Failed to trigger certificate email due to a server error.';
    })
