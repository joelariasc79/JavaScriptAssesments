...
//     async (vaccineData, { rejectWithValue }) => {
//         try {
//             // Adjust the endpoint as per your backend route definition.
//             // Based on your provided router, /api/vaccines seems to be the POST endpoint.
//             const response = await apiService.registerVaccine(vaccineData);
//             return response.data;
//         } catch (error) {
//             const message = error.response?.data?.message || error.message;
...
//         registrationStatus: 'idle', // 'idle' | 'loading' | 'succeeded' | 'failed'
//         registrationError: null,
//         // Optional: for editing/deleting status
//         updateStatus: 'idle',
//         updateError: null,
//     },
//     reducers: {
...
//             .addCase(registerVaccine.fulfilled, (state, action) => {
//                 state.registrationStatus = 'succeeded';
//                 // You might want to add the new vaccine to the 'vaccines' array here
//                 // state.vaccines.push(action.payload.vaccine); // assuming payload contains { vaccine: {...} }
//             })
//             .addCase(registerVaccine.rejected, (state, action) => {
//                 state.registrationStatus = 'failed';
//                 state.registrationError = action.payload;
...
// });
//
// export const { clearRegistrationStatus, clearUpdateStatus } = vaccineSlice.actions;
// export default vaccineSlice.reducer;