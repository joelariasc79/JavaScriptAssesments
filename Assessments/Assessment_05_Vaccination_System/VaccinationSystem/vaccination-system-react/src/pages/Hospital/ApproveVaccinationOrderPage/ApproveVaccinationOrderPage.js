// src/pages/Hospital/ApproveVaccinationOrderPage/ApproveVaccinationOrderPage.js
...
import './ApproveVaccinationOrderPage.css'; // Updated CSS file for table styling
    ...
    return (
        <div className="approve-orders-container">
            ...
            <div className="orders-controls">
                {/* Updated: Wrap search input in a div for icon styling */}
                        ...
                        <table className="orders-table">
                            ...
                            {paginatedOrders.map((order) => (
                                <tr key={order._id}>
                                    ...
                                    <td>{order.userId?.email || order.userId?.username || 'N/A'}</td>
                                    <td>{order.vaccineId?.name || 'N/A'} ({order.vaccineId?.type || 'N/A'})</td>
                                    <td>{order.dose_number}</td>
                                    <td>${order.charge_to_be_paid.toFixed(2)}</td>
                                    <td>{new Date(order.createdAt).toLocaleDateString()}</td>
                                    <td><span className={`status-${order.vaccinationStatus}`}>{order.vaccinationStatus.replace('_', ' ')}</span></td>
                                    <td>
