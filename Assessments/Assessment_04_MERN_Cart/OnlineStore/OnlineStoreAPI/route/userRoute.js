...
// This middleware will be used to protect routes that require a logged-in patient.
function authenticateToken(req, res, next) {
    ...
    if (token == null) {
        // No token provided, patient is not authenticated
    ...
    jwt.verify(token, JWT_SECRET, (err, user) => {
        ...
        // If token is valid, attach the decoded patient payload to the request object
        req.user = user; // patient object contains payload { userId: ..., username: ... }
 ...
 * @description Register a new patient with hashed password
...
userRouter.post('/api/auth/register', async (req, res) => {
    try {
        ...
        // Check if patient already exists by username, userId or email
 ...
 * @description Authenticate patient and return a JWT
...
userRouter.post('/api/auth/login', async (req, res) => {
    try {
        ...
        const payload = {
            ...
            // e.g., roles: patient.roles, department: patient.department
        };
 ...
 * @description Create a new patient. This route is now primarily for registration.
 * If you still need a simple 'create patient' without full registration,
...
userRouter.post('/api/users', async (req, res) => {
    try {
        ...
        // If you intend for this route to *not* handle passwords directly, you need a different patient models or logic.
 ...
 * Note: You might want to add logic to ensure req.patient.userId matches req.params.userId
 * if only a patient can view their own profile.
 */
