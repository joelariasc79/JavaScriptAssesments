...
// --- FIX: Ensure only one, correct definition for DELIVER_ORDER_FAILURE ---
export const DELIVER_ORDER_FAILURE = 'DELIVER_ORDER_FAILURE'; // THIS IS THE CORRECT AND ONLY LINE NEEDED HERE
...
export const deliverOrderFailure = (error) => ({ type: DELIVER_ORDER_FAILURE, payload: error }); // THIS IS THE CORRECTED USAGE
 ...
 * @param {string} token - JWT token for authentication.
 */
export const deliverOrder = (orderId, token) => { // 'token' is your authToken
    return async (dispatch) => {
        dispatch(deliverOrderRequest());
        try {
            const response = await axios.put(
                `${API_BASE_URL}/orders/${orderId}/deliver`,
                {}, // Empty body if no specific data is sent
                {
                    headers: {
