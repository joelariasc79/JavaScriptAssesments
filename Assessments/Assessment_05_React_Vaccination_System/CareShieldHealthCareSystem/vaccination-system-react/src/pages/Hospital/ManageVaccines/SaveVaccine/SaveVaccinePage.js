...
const SaveVaccinePage = () => {
    ...
    const selectedVaccine = useSelector(selectSelectedVaccine);
    ...
    const updateStatus = useSelector(selectUpdateVaccineStatus);
    const updateError = useSelector(selectUpdateVaccineError);

    // State variables for all vaccine fields, including new age fields
    ...
    const [minAgeMonths, setMinAgeMonths] = useState(''); // NEW STATE
    const [maxAgeYears, setMaxAgeYears] = useState('');   // NEW STATE
    ...
    useEffect(() => {
        if (isEditing && fetchByIdStatus === 'succeeded' && selectedVaccine) {
            ...
            setMinAgeMonths(selectedVaccine.min_age_months !== undefined && selectedVaccine.min_age_months !== null ? selectedVaccine.min_age_months.toString() : ''); // NEW
            setMaxAgeYears(selectedVaccine.max_age_years !== undefined && selectedVaccine.max_age_years !== null ? selectedVaccine.max_age_years.toString() : '');   // NEW
    ...
    const handleSubmit = async (e) => {
        ...
        const parsedMinAgeMonths = minAgeMonths !== '' ? parseInt(minAgeMonths) : null; // NEW
        const parsedMaxAgeYears = maxAgeYears !== '' ? parseInt(maxAgeYears) : null;   // NEW
        ...
        // NEW VALIDATION FOR AGE FIELDS
        if (minAgeMonths !== '' && (isNaN(parsedMinAgeMonths) || parsedMinAgeMonths < 0)) {
            setFormError('Minimum Age in Months must be a non-negative number if provided.');
            return;
        }
        if (maxAgeYears !== '' && (isNaN(parsedMaxAgeYears) || parsedMaxAgeYears < 0)) {
            setFormError('Maximum Age in Years must be a non-negative number if provided.');
            return;
        }
        if (parsedMinAgeMonths !== null && parsedMaxAgeYears !== null) {
            if (parsedMaxAgeYears * 12 < parsedMinAgeMonths) {
                setFormError('Maximum age cannot be less than minimum age.');
                return;
            }
        }
        // END NEW VALIDATION
        ...
        const vaccineData = {
            ...
            min_age_months: parsedMinAgeMonths, // NEW
            max_age_years: parsedMaxAgeYears,   // NEW
            ...
            {!(isEditing && fetchByIdStatus === 'loading') && ( // Only show form if not loading existing data
                <form onSubmit={handleSubmit} className="register-vaccine-form">
                    ...
                    <Input
                        ...
                        placeholder="e.g., 6 (for 6 months old)"
                        min="0"
                    />
                    <Input
                        ...
                        placeholder="e.g., 65 (for 65 years old)"
                        min="0"
                    />
                    {/* END NEW AGE FIELDS */}
                    <Input
