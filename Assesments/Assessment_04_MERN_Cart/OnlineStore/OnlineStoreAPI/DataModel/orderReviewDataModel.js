...
const OrderReviewSchema = new mongoose.Schema({
    ...
    order: {
        ...
        ref: 'Order',
        required: true,
        unique: true // Enforce that one order can only have one overall review (from the purchasing user)
    },
    ...
    // Denormalized user info for quicker access without populating 'user'
    reviewerName: {
        type: String,
        required: true,
        trim: true,
...
// Compound unique index to ensure a specific user can review a specific order only once
