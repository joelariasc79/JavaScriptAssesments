 ...
 * @body {string} name, {string} type, {number} price, {string} side_effect, {string} origin, {number} doses_required, {number} time_between_doses_days, {string} other_info, {string} strains_covered
...
vaccineRouter.post('/api/vaccines', authenticateToken, async (req, res) => {
    try {
        ...
        const { name, type, price, side_effect, origin, doses_required, time_between_doses_days, other_info, strains_covered } = req.body;

        // Basic validation for required fields
        if (!name || !type || price === undefined || price === null || !doses_required) {
            return res.status(400).json({ message: 'Missing required vaccine fields: name, type, price, doses_required.' });
        }
